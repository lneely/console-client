  ...
  if (GetAdaptersInfo(AdapterInfo, &dwBufLen) == ERROR_BUFFER_OVERFLOW) {
    ...
    if (AdapterInfo == NULL) {
      ...
      if (GetAdaptersInfo(AdapterInfo, &dwBufLen) == NO_ERROR) {
        sprintf(buffer, "%.2x%.2x%.2x%.2x%.2x%.2x", AdapterInfo->Address[0],
                AdapterInfo->Address[1], AdapterInfo->Address[2],
                AdapterInfo->Address[3], AdapterInfo->Address[4],
...
int create_backend_event(const char *binapi, const char *category,
  ...
  if (pCnt > 0) {
    ...
    for (i = 0; i < pCnt; i++) {
      ...
      if (params->Params[i].paramtype == 2) {
        paramsLocal[mpCnt + i] =
            (binparam)P_BOOL((const char *)charBuff, params->Params[i].num);
        continue;
      }
    }
  ...
  for (i = 0; i < tpCnt; i++) {
    ...
    if (paramsLocal[i].paramtype == 1) {
      debug(D_NOTICE, "%d: Number Param: [%s] - [%d]", i,
            paramsLocal[i].paramname, (int)paramsLocal[i].num);
      continue;
    }
  }
...
#if defined(P_OS_MACOSX)
  int byteRead;
  FILE *stream = popen("system_profiler SPSoftwareDataType | grep \"Computer "
                       "Name\" | cut -d: -f2",
                       "r");
...
int set_be_file_dates(uint64_t fileid, time_t ctime, time_t mtime) {
  ...
  debug(D_NOTICE,
        "Update file date in the backend. FileId: [%lu], ctime: [%ld], "
        "mtime: [%ld]",
        fileid, ctime, mtime);
